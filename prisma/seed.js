...
async function main() {
  ...
  // OwnerProfile will be upserted

  // Souls (members) - from constants/content.ts
  const souls = [
  ...
  for (const s of souls) {
    await prisma.soul.create({ data: s });
  }
  ...
  for (const m of staff) {
    await prisma.staff.create({ data: m });
  }
  ...
  // OwnerProfile - from constants/owner.ts current values
  ...
  console.log('Seeded OwnerProfile');
